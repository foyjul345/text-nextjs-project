...
async function getArticlesByCategory(categoryName: string): Promise<IArticle[]> {
    try {
      const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/article?category=${categoryName}`;
...
async function getArticleByCategory(categoryName: string):Promise<IArticle[]> {
    try {
        const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/article?category=${categoryName}`;
...
const page = async({params}:{params:Promise<{categoryName:string}>}) => {
    ...
    const article = await getArticleByCategory(decodeCategoryName)
    console.log(article)
    return (
 ...
 <div className="container  max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
           ...
           {article.length > 0 ? (
            <div className="">
              <MostRecentSection
                mostRecentArticls={article[0] ? [article[0]] : []} 
                allmostRecentGridArticls={article.slice(1)} 
                popularArticls={article.slice(1, 4)} 
            />
            </div>
          ) : (
            <div className="text-center py-16">
              <h2 className="text-2xl font-semibold text-gray-700">No articles found</h2>
              <p className="text-gray-500 mt-2">There are no articles in the &quot; {decodeCategoryName} &quot; category yet.</p>
